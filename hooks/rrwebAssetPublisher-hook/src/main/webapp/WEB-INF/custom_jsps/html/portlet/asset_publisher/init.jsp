...
<%
	...
	if (selectionStyle.equals("dynamic")) {
		...
		if ((classNameIds.length == 1) && (classTypeIds.length == 1)) {
			AssetRendererFactory assetRendererFactory = AssetRendererFactoryRegistryUtil
					.getAssetRendererFactoryByClassName(PortalUtil.getClassName(classNameIds[0]));

			ddmStructureDisplayFieldValue = GetterUtil
					.getString(portletPreferences.getValue("ddmStructureDisplayFieldValue", StringPool.BLANK));
			ddmStructureFieldName = GetterUtil
					.getString(portletPreferences.getValue("ddmStructureFieldName", StringPool.BLANK));
			ddmStructureFieldValue = portletPreferences.getValue("ddmStructureFieldValue", StringPool.BLANK);

			if (Validator.isNotNull(ddmStructureFieldName) && Validator.isNotNull(ddmStructureFieldValue)) {
				List<Tuple> classTypeFieldNames = assetRendererFactory.getClassTypeFieldNames(classTypeIds[0],
						locale, QueryUtil.ALL_POS, QueryUtil.ALL_POS);
					...
					if (fieldName.equals(ddmStructureFieldName)) {
						...
						if (subtypeFieldsFilterEnabled) {
							...
							assetEntryQuery.setAttribute("ddmStructureFieldName",
									DDMIndexerUtil.encodeName(ddmStructureId, ddmStructureFieldName, locale));
	...
	long assetVocabularyId = GetterUtil
			.getLong(portletPreferences.getValue("assetVocabularyId", StringPool.BLANK));

	long assetCategoryId = ParamUtil.getLong(request, "categoryId");

	String assetCategoryIdString = "" + assetCategoryId;

	Cookie[] cookies = request.getCookies();

	boolean hasBeenInBrowser = false;
	if (cookies != null) {
		for (Cookie cookie : cookies) {
			if (cookie.getName().equals("liferay_regionId")) {
				hasBeenInBrowser = true;

				String cookieValue = cookie.getValue();

				if (!cookieValue.equalsIgnoreCase(assetCategoryIdString)) {
					Cookie regionIdCookie = new Cookie("liferay_regionId", cookieValue);

					regionIdCookie.setMaxAge(60 * 60 * 24 * 7);
					regionIdCookie.setPath(StringPool.SLASH);

					renderResponse.addProperty(regionIdCookie);
					response.addCookie(regionIdCookie);
					if (assetCategoryId == 0) {
						assetCategoryId = Long.parseLong(cookieValue);
					}
				} 
	...
	} else if (!hasBeenInBrowser) {

		Cookie regionIdCookie = new Cookie("liferay_regionId", assetCategoryIdString);
	...
	if (assetCategoryId > 0) {
		...
		long allRegionAssetCategoryId = 29596;
		if (selectionStyle.equals("dynamic")) {
			allAssetCategoryIds = assetEntryQuery.getAllCategoryIds();
			//add category all regions to assetQuery
		...
		if (assetCategory != null) {
			...
			assetCategory = AssetCategoryLocalServiceUtil.fetchCategory(allRegionAssetCategoryId);
			if (assetCategory != null) {
	...
	boolean mergeLayoutTags = GetterUtil.getBoolean(portletPreferences.getValue("mergeLayoutTags", null),
			false);

	String displayStyle = GetterUtil.getString(
			portletPreferences.getValue("displayStyle", PropsValues.ASSET_PUBLISHER_DISPLAY_STYLE_DEFAULT));
	long displayStyleGroupId = GetterUtil.getLong(portletPreferences.getValue("displayStyleGroupId", null),
			themeDisplay.getScopeGroupId());

	boolean showAddContentButton = GetterUtil
			.getBoolean(portletPreferences.getValue("showAddContentButton", null), true);
	...
	int pageDelta = GetterUtil.getInteger(
			portletPreferences.getValue("pageDelta", portletPreferences.getValue("delta", null)),
	...
	boolean showAvailableLocales = GetterUtil
			.getBoolean(portletPreferences.getValue("showAvailableLocales", null));
	boolean showMetadataDescriptions = GetterUtil
			.getBoolean(portletPreferences.getValue("showMetadataDescriptions", null), true);

	String defaultAssetPublisherPortletId = AssetUtil.getDefaultAssetPublisherId(layout);

	boolean defaultAssetPublisher = AssetUtil.isDefaultAssetPublisher(defaultAssetPublisherPortletId,
			portletDisplay.getId(), portletResource);

	boolean enablePermissions = _isEnablePermissions(portletName, portletPreferences);

	assetEntryQuery.setEnablePermissions(enablePermissions);

	boolean enableRelatedAssets = GetterUtil
			.getBoolean(portletPreferences.getValue("enableRelatedAssets", null), true);
	...
	boolean enableCommentRatings = GetterUtil
			.getBoolean(portletPreferences.getValue("enableCommentRatings", null));
	boolean enableTagBasedNavigation = GetterUtil
			.getBoolean(portletPreferences.getValue("enableTagBasedNavigation", null));
	boolean enableViewCountIncrement = GetterUtil
			.getBoolean(portletPreferences.getValue("enableViewCountIncrement", null), true);
	...
	boolean enableSocialBookmarks = GetterUtil
			.getBoolean(portletPreferences.getValue("enableSocialBookmarks", null), true);
	String socialBookmarksDisplayStyle = portletPreferences.getValue("socialBookmarksDisplayStyle",
			"horizontal");
	String socialBookmarksDisplayPosition = portletPreferences.getValue("socialBookmarksDisplayPosition",
			"bottom");
	...
	String[] metadataFields = StringUtil
			.split(portletPreferences.getValue("metadataFields", defaultMetadataFields));

	boolean enableRSS = !PortalUtil.isRSSFeedsEnabled() ? false
			: GetterUtil.getBoolean(portletPreferences.getValue("enableRss", null));
	int rssDelta = GetterUtil.getInteger(portletPreferences.getValue("rssDelta", StringPool.BLANK),
			SearchContainer.DEFAULT_DELTA);
...
<%!private Set<Group> _getAncestorSiteGroups(long groupId, boolean checkContentSharingWithChildrenEnabled)
	...
	private List<Group> _getCurrentAndAncestorSiteGroups(long groupId, boolean checkContentSharingWithChildrenEnabled)
			throws PortalException, SystemException {
		Set<Group> groups = new LinkedHashSet<Group>();
	...
	private List<Group> _getCurrentAndAncestorSiteGroups(long[] groupIds,
			boolean checkContentSharingWithChildrenEnabled) throws PortalException, SystemException {
		Set<Group> groups = new LinkedHashSet<Group>();
	...
	private boolean _isEnablePermissions(String portletName, PortletPreferences portletPreferences) {
		if (!portletName.equals(PortletKeys.HIGHEST_RATED_ASSETS) && !portletName.equals(PortletKeys.MOST_VIEWED_ASSETS)
